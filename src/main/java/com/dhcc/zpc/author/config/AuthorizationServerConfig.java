 ...
 *
 * 继承AuthorizationServerConfigurerAdapter，完成对授权服务器的配置，然后通过@EnableAuthorizationServer注解
 *      开启授权服务器。
 * AuthenticationManager对象用来支持password授权模式。
 * RedisConnectionFactory对象用来完成redis的缓存。
 * UserDetailsService对象为刷新token提供支持。
 * authorizedGrantTypes()方法配置授权模式为password和refresh_token，其实在标准的oauth中并没有refresh_token，
 *      但是spring security中将其归为一种，因此要实现access_token的刷新就需要refresh_token。
